...
@export var barrack_price = [0,10,0,0,0]
@export var spear_tower_price = [0,10,5,0,0]
@export var archer_range_price = [0,20,2,0,0]
@export var stable_price = [10,10,0,0,0]
@export var castle_price = [10,10,10,0,0]
@export var tavern_price = [10,10,2,2,0]
@export var farm_price = [10,0,0,0,0]
...
signal update_ui_resources(resources:Array)
func check_and_spend_resources(prices:Array) -> bool:
	var resources = get_parent().resources
	for i in range(len(prices)):
		if resources[i] < prices[i]:
			return false

	for i in range(len(prices)):
		get_parent().resources[i] = resources[i] - prices[i]

	update_ui_resources.emit(resources)
	return true

func _on_button_pressed() -> void:
	if check_and_spend_resources(building_prices[0]):
		scene_to_build = building_scenes[0]
		$ghost.region_rect = Rect2(3*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_2_pressed() -> void:
	if check_and_spend_resources(building_prices[1]):
		scene_to_build = building_scenes[1]
		$ghost.region_rect = Rect2(6*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_3_pressed() -> void:
	if check_and_spend_resources(building_prices[2]):
		scene_to_build = building_scenes[2]
		$ghost.region_rect = Rect2(0*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_4_pressed() -> void:
	if check_and_spend_resources(building_prices[3]):
		scene_to_build = building_scenes[3]
		$ghost.region_rect = Rect2(2*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_5_pressed() -> void:
	if check_and_spend_resources(building_prices[4]):
		scene_to_build = building_scenes[4]
		$ghost.region_rect = Rect2(1*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_6_pressed() -> void:
	if check_and_spend_resources(building_prices[5]):
		scene_to_build = building_scenes[5]
		$ghost.region_rect = Rect2(4*64, 0, 64, 44)
		$ghost.visible = true

func _on_button_7_pressed() -> void:
	if check_and_spend_resources(building_prices[6]):
